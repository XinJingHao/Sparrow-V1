...
'''Sparrow-V1.2 Configuration
from SparrowV1_2.core import Sparrow, str2bool
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--map_address', type=str, default='train_maps', help='map address: train_maps / test_maps')
parser.add_argument('--device', type=str, default='cpu', help='running device of Sparrow: cuda / cpu')
...
opt.state_dim = 4+opt.grouped_ld_num # [D2T,alpha,v_linear,v_angular] + [lidar result]
'''
...
Current version: V1.2 (2024/12/03)
'''
...
class Sparrow():
    ...
    def __init__(self, **params):
        ...
        self.maps = np.sort(os.listdir(os.getcwd() + '/SparrowV1_2/' + self.map_address))
        ...
        self.target_area = 50 # # enter the target_area×target_area (cm) space in the upper left of the map will be considered win.
        self.target_point = torch.tensor([[self.target_area/2, self.target_area/2]]).repeat(self.N, 1).to(self.dvc)  # (N,2)
        self.D = 400 # distance2target normalizer
        ...
        self.ctrl_delay = 0 # in ctrl_interval
        ...
        if self.noise:
            self.noise_magnitude = torch.hstack((torch.tensor([[2,2,torch.pi/50,1,torch.pi/50]]), torch.ones(self.grouped_ld_num))).to(self.dvc) #(32,)
        ...
        self.state_upperbound[0] *= self.D  # maximal LPP distance of Sparrow-V3.3
        ...
        self.state_upperbound[4:self.state_dim] *= self.ld_range
    ...
    def _bound_init(self):
        ...
        for map_name in self.maps: # ['map1.png' 'map10.png' 'map11.png' ... 'map14.png' 'map15.png' 'map2.png' 'map3.png' ... 'map9.png']
            map_pyg = pygame.image.load(os.getcwd() + '/SparrowV1_2/' + self.map_address + '/' + map_name) # 不能用plt.imread读, 有bug
    ...
    def _Normalize(self, observation):
        '''Normalize the raw observations (N,5+ld_num) to relative observations (N,4+ld_num)'''
        # 1) Normalize the orientation -> alpha:
        ...
        self.d2target_now = (self.car_state[:, 0:2] - self.target_point).pow(2).sum(dim=-1).pow(0.5)  # (N,)
        new_obs = torch.hstack((self.d2target_now.unsqueeze(-1), observation[:,2:])) # (N,31), [D2T,alpha,Vlinear,Vangle,ld0,...ldn]

        # 3) Normalize new_obs:
        return new_obs/self.state_upperbound

    def _get_obs(self):
        '''Return: [d2target, alpha, v_linear, v_angular, lidar_results(0), ..., lidar_results(26)] in shape (N,32) '''
        ...
        return self._Normalize(observation_vec) # (N,5+ld_num) to (N,4+ld_num)
    ...
    def _render_frame(self):
        ...
        if self.canvas is None :
            ...
            self.map = pygame.image.load(os.getcwd() + '/SparrowV1_2/'+self.map_address+'/' + self.maps[self.ri])
