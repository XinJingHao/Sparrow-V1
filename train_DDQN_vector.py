...
parser.add_argument('--write', type=str2bool, default=True, help='Whether use SummaryWriter to record the training curve')
parser.add_argument('--Loadmodel', type=str2bool, default=False, help='Whether load pretrained model')
parser.add_argument('--ModelIdex', type=int, default=473, help='which model(e.g. 473k.pth) to load')
...
parser.add_argument('--ld_GN', type=int, default=1, help='how many lidar streams are grouped for one group')
...
parser.add_argument('--compile', type=str2bool, default=True, help='whether to torch.compile to boost simulation speed')
opt = parser.parse_args()

opt.actor_envs = len(os.listdir(os.getcwd() + '/SparrowV1_2/' + opt.map_address))
...
def main(opt):
	...
	if opt.render: # render with Sparrow
		...
		if opt.write:
			...
			writepath = f'runs/SparrowV1.2_DDQN_for_ColorDynamic' + timenow
		...
		eval_train_envs = Sparrow(**vars(eval_confg))
		eval_confg.map_address = 'test_maps'
		eval_test_envs = Sparrow(**vars(eval_confg))
		...
		while total_steps < opt.Max_train_steps:
			...
			if total_steps % (opt.eval_freq*opt.actor_envs) == 0:
				_, train_arrival_rate = evaluate_policy(eval_train_envs, model, deterministic=True, AWARD=opt.AWARD)
				_, test_arrival_rate = evaluate_policy(eval_test_envs, model, deterministic=True, AWARD=opt.AWARD)
				print(f'Sparrow-V1.2  N:{opt.actor_envs};  Total steps: {round(total_steps / 1e3, 2)}k;  Train Arrival rate:{train_arrival_rate};  Test Arrival rate:{test_arrival_rate}')
				if opt.write:
		...
		envs.close()
		eval_train_envs.close()
		eval_test_envs.close()
...
class DDQN_Agent(object):
	def __init__(self,opt):
		...
		self.p = torch.ones(opt.actor_envs, device=self.dvc)
	...
	def fresh_explore_prob(self, steps):
		...
		explore = torch.arange(i, device=self.dvc) / (2 * i)  # 0 ~ 0.5
	...
	def select_action(self, s, deterministic):
		...
		with torch.no_grad():
			...
			if deterministic:
				...
				replace = torch.rand(self.actor_envs, device=self.dvc) < self.p  # [n]
				rd_a = torch.randint(0, self.action_dim, (self.actor_envs,), device=self.dvc)
